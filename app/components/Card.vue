<template>
    <a :class="`block group border-2 border-gray-700 hover:border-gray-300/30 rounded-xl sm:hover:-translate-y-1 transition duration-300 ease-in-out`"
        :href="props.href">
        <article class="relative flex flex-col justify-between w-full py-6 px-7">
            <h2 class="text-gray-200 text-lg font-bold">
                {{ props.title }}
            </h2>
            <p class="text-slate-200">
                {{ props.description }}
            </p>
            <div class="flex gap-1 mt-1.5 items-center">
                <ButtonTooltip label="Read more" tag="a" href="#">
                    <IconsCopy />
                </ButtonTooltip>
            </div>
            <div
                class="absolute grid place-items-center right-3 top-3 bg-gray-800 w-9 h-9 rounded-full scale-0 group-hover:scale-90 transition-transform">
                <div class="text-gray-50 text-2xl">
                    <IconsExpand />
                </div>
            </div>
        </article>
    </a>
</template>

<script setup lang="ts">
import ButtonTootip from '~/components/ButtonTooltip.vue';

const props = defineProps<{
    title: string;
    description: string;
    href?: string;
}>();

</script>